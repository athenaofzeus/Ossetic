...
from flask import request, render_template, url_for, session, jsonify, make_response
from Ossetic.models import db, Users, Roles, Entry_logs, Units, Forms
...
def profile(user_id=None):
    ...
    if request.method == 'GET':
        ...
        unique_edits = list()
        included_unit_ids = []
        for l in Entry_logs.query.filter(Entry_logs.user_id == user_id, Entry_logs.event != 3).order_by(
                Entry_logs.log_id.desc()).all():
...
def profile_info():
    ...
    if not session.get('user'):
        return Check.login()
    who = URLSafeSerializer(app.config["SECRET_KEY"], salt='login').loads(session.get('user'))
    if Users.query.get(who).role_id not in [2, 3]:
        return Check.status()
    ...
    if request.method == 'POST':
        ...
        response = []
        if type == 'recent':
            for i in Entry_logs.query.order_by(Entry_logs.log_id.desc()).filter(Entry_logs.user_id==user_id, Entry_logs.event != 3, Entry_logs.log_id < last).limit(30).all():
                response.append([Amend.datetime(i.datetime), Amend.cypher_unit(i.unit_id), i.unit_id,
                                 Check.main_form(i.unit_id), get_type(i.event), i.log_id])
        elif type == 'unique':
            unique_edits = list()
            for i in Entry_logs.query.order_by(Entry_logs.log_id.desc()).\
                    filter(Entry_logs.user_id == user_id, Entry_logs.event != 3, Entry_logs.log_id not in unique_edits,\
                           Entry_logs.log_id < last).limit(30).all():
                unique_edits.append(i.log_id)
                response.append([Amend.datetime(i.datetime), Amend.cypher_unit(i.unit_id), i.unit_id,
                                 Check.main_form(i.unit_id), get_type(i.event), i.log_id])
